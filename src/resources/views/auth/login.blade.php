
<x-guest-layout>
    <!-- セッションステータス表示 -->
    <x-auth-session-status class="mb-4" :status="session('status')" />

    <section class="w-full min-h-screen flex items-center justify-center dark:bg-gray-800">
        <div class="w-full  flex flex-col items-center justify-center">
            <h1 class="text-2xl mb-6 text-white text-center">ブックログ</h1>

            <div class="w-fullh   bg-white rounded-lg shadow dark:border sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    <h2 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
                        アカウントでサインイン
                    </h2>

                    <!-- 通常ログインフォーム -->
                    <form class="space-y-4 md:space-y-6" method="POST" action="{{ route('login') }}">
                        @csrf
                        <div>
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">メールアドレス</label>
                            <input type="email" name="email" id="email" required placeholder="name@example.com" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-teal-600 focus:border-teal-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
>
                        </div>
                        <div>
                            <label for="password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">パスワード</label>
                            <input type="password" name="password" id="password" required placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-teal-600 focus:border-teal-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
>
                        </div>
                        <div class="flex items-center justify-between">
                            <label for="remember" class="flex items-center text-sm text-gray-500 dark:text-gray-300">
                                <input id="remember" type="checkbox" name="remember" class="mr-2"> 次回以降自動でログイン
                            </label>
                            <a href="{{ route('password.request') }}" class="text-sm text-teal-600 hover:underline">パスワードを忘れた？</a>
                        </div>
                        <button type="submit" class="text-white bg-teal-600 py-2 px-4 rounded w-full">サインイン</button>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            アカウント登録は <a href="{{ route('register') }}" class="text-teal-600 hover:underline">こちら</a>
                        </p>
                    </form>

                    <!-- Googleログインボタン -->
                    <form method="GET" action="{{ route('google.redirect') }}">
                        <button type="submit" class= "w-full inline-flex items-center justify-center py-2 px-4 text-sm font-medium  text-black bg-white hover:bg-red-100 rounded">
                            <svg class="w-5 h-5 mr-2" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
  <g clip-path="url(@html('#clip0_13183_10121'))">
    <path d="M20.3081 10.2303C20.3081 9.55056 20.253 8.86711 20.1354 8.19836H10.7031V12.0492H16.1046C15.8804 13.2911 15.1602 14.3898 14.1057 15.0879V17.5866H17.3282C19.2205 15.8449 20.3081 13.2728 20.3081 10.2303Z" fill="#3F83F8"/>
    <path d="M10.7019 20.0006C13.3989 20.0006 15.6734 19.1151 17.3306 17.5865L14.1081 15.0879C13.2115 15.6979 12.0541 16.0433 10.7056 16.0433C8.09669 16.0433 5.88468 14.2832 5.091 11.9169H1.76562V14.4927C3.46322 17.8695 6.92087 20.0006 10.7019 20.0006Z" fill="#34A853"/>
    <path d="M5.08857 11.9169C4.66969 10.6749 4.66969 9.33008 5.08857 8.08811V5.51233H1.76688C0.348541 8.33798 0.348541 11.667 1.76688 14.4927L5.08857 11.9169Z" fill="#FBBC04"/>
    <path d="M10.7019 3.95805C12.1276 3.936 13.5055 4.47247 14.538 5.45722L17.393 2.60218C15.5852 0.904587 13.1858 -0.0287217 10.7019 0.000673888C6.92087 0.000673888 3.46322 2.13185 1.76562 5.51234L5.08732 8.08813C5.87733 5.71811 8.09302 3.95805 10.7019 3.95805Z" fill="#EA4335"/>
  </g>
  <defs>
    <clipPath id="clip0_13183_10121">
      <rect width="20" height="20" fill="white" transform="translate(0.5)"/>
    </clipPath>
  </defs>
</svg>

                            Googleアカウントでサインイン
                        </button>
                    </form>
                </div>
            </div>
        </div>
    </section>
</x-guest-layout>
